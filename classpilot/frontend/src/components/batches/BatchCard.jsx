    ...
    >
      ...
      <div className="mb-3 flex items-start justify-between">
        ...
        <div className="flex shrink-0 gap-1">
          <button onClick={() => onEdit(batch)} className="rounded-lg p-1.5 text-ink/35 hover:bg-iris-50 hover:text-iris-600">
            <Pencil size={14} />
          </button>
          <button onClick={() => onDelete(batch)} className="rounded-lg p-1.5 text-ink/35 hover:bg-blossom-50 hover:text-blossom-600">
            <Trash2 size={14} />
          </button>
        </div>
      </div>
