                ...
                return (
                  <button
                    ...
                    className={`flex h-20 flex-col items-start gap-1 rounded-xl border p-1.5 text-left transition-colors ${
                      selectedDay === d
                        ? "border-petrol-400 bg-petrol-50"
                        : isToday(d)
                        ? "border-petrol-200 bg-white"
                        : "border-ink/5 bg-white hover:border-petrol-200"
                    }`}
                  >
                    <span className={`text-xs font-bold ${isToday(d) ? "text-petrol-600" : "text-ink/60"}`}>{d}</span>
                    <div className="flex flex-wrap gap-0.5">
                      {info.classes.slice(0, 4).map((b, i) => (
                        <span key={i} className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: b.colorTag }} />
                      ))}
                      {info.fees.length > 0 && <IndianRupee size={9} className="text-ochre-500" />}
                      {info.birthdays.length > 0 && <Cake size={9} className="text-oxblood-500" />}
                    </div>
                  </button>
                );
              })}
            ...
            {!selected ? (
              ...
              <div className="flex flex-col gap-4">
                {selected.classes.length > 0 && (
                  <div>
                    <p className="mb-1.5 text-xs font-bold text-ink/45">Classes</p>
                ...
                {selected.fees.length > 0 && (
                  <div>
                    <p className="mb-1.5 text-xs font-bold text-ink/45">Fee Due</p>
                    {selected.fees.map((f) => (
                      <div key={f._id} className="mb-1.5 flex items-center justify-between rounded-xl bg-ochre-50 px-3 py-2 text-sm">
                        <span className="font-medium text-ink">{f.student?.fullName}</span>
                        <span className="font-mono text-xs text-ochre-700">{formatCurrency(f.amount)}</span>
                      </div>
                    ))}
                  </div>
                )}
                {selected.birthdays.length > 0 && (
                  <div>
                    <p className="mb-1.5 text-xs font-bold text-ink/45">Birthdays 🎂</p>
                    {selected.birthdays.map((s) => (
                      <div key={s._id} className="mb-1.5 flex items-center gap-2 rounded-xl bg-oxblood-50 px-3 py-2 text-sm">
                        <Cake size={14} className="text-oxblood-500" />
