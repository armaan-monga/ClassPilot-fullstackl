...
export default function BirthdayList({ birthdays = [] }) {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {birthdays.map((b, i) => (
        <div key={i} className="flex items-center gap-3 rounded-2xl bg-oxblood-50/60 px-3 py-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-oxblood-500 text-xs font-bold text-white">
            {initials(b.name)}
          </div>
          ...
          <Cake size={16} className="text-oxblood-400" />
        </div>
      ))}
    </div>
  );
