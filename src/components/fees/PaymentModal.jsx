...
export default function PaymentModal({ open, onClose, onUpdated, fee, instituteName, currency }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={`${fee.student?.fullName} — ${monthLabel}`} width="max-w-md">
      <div className="mb-4 rounded-2xl bg-petrol-50 p-4 text-center">
        <p className="text-xs font-semibold text-petrol-700">Amount Due</p>
        <p className="font-display text-2xl font-bold text-petrol-700">{formatCurrency(remaining, currency)}</p>
      </div>

      <div className="mb-4 flex gap-2">
        <button
          onClick={() => setMode("full")}
          className={`flex-1 rounded-xl py-2 text-sm font-semibold ${mode === "full" ? "bg-petrol-500 text-white" : "bg-ink/5 text-ink/60"}`}
        >
          Pay in Full
        </button>
        <button
          onClick={() => setMode("partial")}
          className={`flex-1 rounded-xl py-2 text-sm font-semibold ${mode === "partial" ? "bg-petrol-500 text-white" : "bg-ink/5 text-ink/60"}`}
        >
          Partial Payment
        </button>
      </div>
      ...
      <div className="mt-5 border-t border-ink/5 pt-4">
        <p className="mb-2 text-xs font-semibold text-ink/50">Send a Reminder Instead</p>
        <Button
          variant="secondary"
          icon={Bell}
