...
export default function Topbar({ onMenuClick }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-ink/5 bg-paper/80 px-4 py-3 backdrop-blur-lg sm:px-6">
      ...
      <div ref={searchRef} className="relative flex-1 max-w-md">
        ...
        <input
          ...
          className="w-full rounded-full border border-ink/10 bg-white py-2.5 pl-10 pr-4 text-sm outline-none focus:border-petrol-300"
        />
        {results && (
          <div className="absolute left-0 right-0 top-12 z-40 max-h-80 overflow-y-auto rounded-2xl border border-ink/5 bg-white p-2 shadow-xl scrollbar-thin">
            ...
            {results.students.length > 0 && (
              <div className="mb-1">
                <p className="px-3 py-1 text-[11px] font-bold text-ink/45">Students</p>
                {results.students.map((s) => (
                  <button
                    key={s._id}
                    onClick={() => goTo(`/students/${s._id}`)}
                    className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left text-sm hover:bg-petrol-50"
                  >
                    <User size={15} className="text-petrol-500" />
            ...
            {results.batches.length > 0 && (
              <div>
                <p className="px-3 py-1 text-[11px] font-bold text-ink/45">Batches</p>
      ...
      <div className="ml-auto" ref={profileRef}>
        <button
          ...
          className="flex items-center gap-2 rounded-full border border-ink/10 bg-white py-1.5 pl-1.5 pr-3 hover:border-petrol-300"
        >
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-petrol-500 text-xs font-bold text-white">
        ...
        {profileOpen && (
          <div className="absolute right-4 mt-2 w-48 rounded-2xl border border-ink/5 bg-white p-1.5 shadow-xl sm:right-6">
            <button
              ...
              className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm font-medium text-ink hover:bg-petrol-50"
            >
              <User size={15} /> Profile & Settings
            </button>
            <button
              onClick={logout}
              className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm font-medium text-oxblood-600 hover:bg-oxblood-50"
            >
